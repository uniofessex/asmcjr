---
title: "Similarities and Dissimilarities Data"
# author: "Yen-Chieh Liao"
# date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Similarities and Dissimilarities Data}
...
# Calculate the column means of ches_eu, ignoring NA values
means <- colMeans(ches_eu, na.rm = TRUE)

# Calculate the standard deviations of the columns in ches_eu, ignoring NA values
sds2 <- apply(ches_eu, 2, sd, na.rm = TRUE)

# Convert ches_eu to a matrix
ches_eu <- as.matrix(ches_eu)

# Ensure the mode of ches_eu is numeric
mode(ches_eu) <- "numeric"
...
```{r}
library(basicspace)
result <- blackbox_transpose(ches_eu,dims=3,
...
europe.dat <- data.frame(
  ...
  party = colnames(ches_eu),  # Use the column names of ches_eu as party names
